...
// Returns SNTP_SUCCESS or SNTP_NOREPLY in case of a network error
int ntp_exchange(const char* host, int port, struct ntp_packet *tx,
...
int get_ntp_timestamp(char *ntp_host, uint64_t *timestamp)
{
    ...
    if(ntp_exchange(ntp_host, dst_ntp_port, &tx, &rx) != SNTP_SUCCESS)
        return SNTP_NETWORK;
